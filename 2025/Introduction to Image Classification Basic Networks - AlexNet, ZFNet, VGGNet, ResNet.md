...
- **큰 Convolution 필터 (11×11, 5×5)**  
  CNN에서 “11×11”, “5×5”, “3×3” 같은 필터 크기(kernel size)는 네트워크가 이미지를 어느 영역 단위로 바라보는지를 결정하는 핵심 요소  
- 두 개의 GPU로 병렬 학습
...
- 깊게 만들수록 성능이 떨어지는 **Degradation Problem**
- (Overfitting이 아니라 **Gradient 흐름 자체가 무너짐**)
...
- 2개의 GPU 병렬연산  
- Max Pooling: stride 2를 주어 윈도우가 겹치도록 풀링  
- ReLU 함수  
- Dropout  
- Augmentation  
...
- 이미 존재하는 CNN을 **가시화**하여 개선 방향을 파악  
- Unpooling 시 switch 활용  
- AlexNet의 11×11 필터 → ZFNet에서는 7×7 필터로 변경  
...
[2] 발표 논문 : Image Classification Task의 기반이 되는 논문 4개  
